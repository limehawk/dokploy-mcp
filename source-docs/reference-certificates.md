...
## Request Body
application/json
certificateId?stringname*stringLength
```
1 <= length
```
certificateData*stringLength
```
1 <= length
```
privateKey*stringLength
```
1 <= length
```
certificatePath?stringautoRenew?boolean|nullorganizationId*stringserverId?string|null[key: string]?never
## Response Body
### 200application/json
...
cURLJavaScriptGoPythonJavaC#
```
curl -X POST "https://your-dokploy-instance.com/api/certificates.remove" \
-H "x-api-key: " \
-H "Content-Type: application/json" \
-d '{
...
```
[BitbucketPrevious Page](/docs/api/reference-bitbucket)[ClusterNext Page](/docs/api/reference-cluster)