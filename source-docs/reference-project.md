...
1 <= length
```
description?string|nullenv?string[key: string]?never
## Response Body
### 200application/json
...
1 <= length
```
description?string|nullcreatedAt?stringorganizationId?stringenv?string[key: string]?never
## Response Body
### 200application/json
### defaultapplication/json
cURLJavaScriptGoPythonJavaC#
...
## Request Body
application/json
sourceEnvironmentId*stringname*stringdescription?stringincludeServices?booleanDefault
```
true
```
selectedServices?array<object>duplicateInSameProject?booleanDefault
```
false
```
[key: string]?never
...
```
[Preview DeploymentPrevious Page](/docs/api/reference-previewDeployment)[RedirectsNext Page](/docs/api/reference-redirects)